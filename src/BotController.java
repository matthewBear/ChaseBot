...
public class BotController {
    ...
    private final int LOOP_SLEEP_TIME = 25;
    private final String POST_URL="http://localhost:1080/";

    public BotController() throws IOException {
     ...
     */
    private void startShowingControllerData() throws ClientProtocolException, IOException{
        while(true)
        {
            ...
            // set x and y axes,
            System.out.println(xAxisPercentage + ":" + yAxisPercentage);

            window.setXYAxis(xAxisPercentage, yAxisPercentage);
            // add other axes panel to window.
            window.addAxisPanel(axesPanel);
            
            //Prepare the JSON message, and then send it
            XYTuple xyValue = new XYTuple(xAxisPercentage,yAxisPercentage);
            String json = gson.toJson(xyValue);

			HttpPost post = new HttpPost(POST_URL);
			post.setEntity(new StringEntity(json));
			post.setHeader("Content-type", "application/json");
			
			//Commented out because I'm getting connection refused when testing
